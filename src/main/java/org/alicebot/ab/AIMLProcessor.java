...
import com.test.autobot.service.FunctionService;
...
public class AIMLProcessor {
	...
	public static String respond(String input, String that, String topic, Chat chatSession, int srCnt) {
		...
		try {
			...
			response = evalTemplate(leaf.category.getTemplate(), ps, chatSession.hashCode());
			// log.info("That="+that);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private static String srai(Node node, ParseState ps) {
		...
		try {
			...
			response = evalTemplate(leaf.category.getTemplate(), new ParseState(ps.depth + 1, ps.chatSession, ps.input, ps.that, topic, leaf), ps.chatSession.hashCode());
			// log.info("That="+that);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	private static String fetch(Node node, ParseState ps) {
		...
		try{
			...
			for (int i = 0; i < childList.getLength(); i++) {
				Node child = childList.item(i);
				param += recursEval(child, ps);
			}
		} catch(Exception e) {
			log.error("Something went wrong with fetchTag", e.getMessage());
		}
		if(method != null) {
			FunctionService functionService = new FunctionService();
			if("orderStatus".equals(method)) {
				return functionService.getOrderStatus(param);
			} else if("nearestStore".equals(method)) {
				return functionService.nearestStore(param);
			} else if("trackOrder".equals(method)){
				return functionService.trackOrder(param);
			} else if("dispatchDate".equals(method)) {
				return functionService.dispatchDate(param);
			} else if("deliveryDate".equals(method)) {
				return functionService.deliveryDate(param);
			}
		}
		return null;		
	}
		...
		// otherwise this is a <condition> with <li> items:
			...
			for (int i = 0; i < liList.size() && result.equals(""); i++) {
				...
				if (value != null) {
					...
					else if (liVarName != null && value != null && (ps.vars.get(liVarName).equals(value) || (ps.vars.containsKey(liPredicate) && value.equals("*"))))
						return evalTagContent(n, ps, attributeNames);
	...
	private static String recursEval(Node node, ParseState ps) {
		try {
			...
			else if (nodeName.equals("fetch"))
				return fetch(node, ps);
	 ...
	 */
	private static String evalTemplate(String template, ParseState ps, Integer hashCode) {
